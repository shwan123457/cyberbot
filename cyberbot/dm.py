...
async def handle_dm(user, msg=None):
    ...
    async with msg.channel.typing():
        ...
        user = discord.utils.get(
            client.guild.members, name=user.name, discriminator=user.discriminator
        )
        ...
        if pieces[0] == "!nominate":
            if client.election and client.election.nomination_started:
                ...
                if pieces[1] in ("accept", "reject", "cancel", "second"):
                    return
                tosend = (
                    await client.election.handle_nominate(user, pieces[1]) or tosend
                )
        elif pieces[0] == "!vote":
            if not is_member_in_voice_channel(
                user, "📋 meetings"
            ):  # must be present to vote/nominate
                await send_dm(
                    user,
                    "You must be in the meetings voice channel to vote during this election.",
                )
                return
            ...
            tosend = (
                await member_stats(user, pieces[1] if len(pieces) > 1 else "") or tosend
            )
        elif pieces[0] == "!dump":
            tosend = (
                await get_channel_messages(user, pieces[1] if len(pieces) > 1 else "")
                or tosend
            )
        elif pieces[0] == "!nonmembers":
            tosend = await get_nonmembers(user) or tosend
        elif (
            pieces[0] == "!flag" and client.datafile
        ):  # make sure datafile set before doing anything with flags
            tosend = (
                await process_flag(user, pieces[1] if len(pieces) > 1 else None)
                or tosend
            )
        ...
        if (
            client.election
            and tosend == errorMsg
            and user.id in client.election.usersWaitingForStatement
        ):  # don't error out when nominee sends campaign statement
            early_exit = True
    if early_exit:
        return
...
async def delete_nominee(user, msg):
    ...
    nominee = discord.utils.get(
        client.guild.members, name=msg.split("#")[0], discriminator=msg.split("#")[1]
    )
    if not nominee:
    ...
    await send_dm(
        nominee,
        f"<@{user.id}> is attempting to delete your nomination for candidacy. Do you accept? [y/n]",
    )
...
async def member_stats(user, command):
    ...
    member = discord.utils.get(
        ...
        discriminator=command[0].split("#")[1],
    )
        ...
        for h in history:
            if isinstance(h.content, str):
                ...
                if not ("flags=<MessageFlags " in h.content):
                    ...
                    content = clean_vote_message(
                        h.content, h.author
                    )  # redact usernames from vote/nomination messages
...
async def get_channel_messages(user, params):
    ...
    if not (
        channel.permissions_for(me).read_message_history
    ...
    for i, h in enumerate(history):
        ...
        if (
            (not last_time)
            or (current_time.day != last_time.day)
            or (current_time.month != last_time.month)
            or (current_time.year != last_time.year)
        ):
    ...
    all_history.reverse()
    newfile = make_file(
        f"{client.guild.name}-{channel.name}-{arrow.utcnow().format('YYYY-MM-DD_HH-mm-ss')}.txt".replace(
            " ", "_"
        ),
        "\n".join(all_history),
    )
...
async def alert_nonmembers():
    ...
    accept_rules_id = discord.utils.get(
        client.guild.channels, name="accept-rules-here"
    ).id
    for member_id in client.non_members:
        member = discord.utils.get(client.guild.members, id=member_id)
        await send_dm(
            member,
            (
                f"Hi! We at the {client.clubname} noticed you hadn't accepted"
...
async def process_flag(user, msg):
    ...
    if msg == None or msg == "help":
        return usage
    command, params = (
        msg.split(" ", 1) if msg.strip().count(" ") > 0 else (msg.split(" ", 1)[0], "")
    )
    if command == "list":
        flag_list = "flags:\n"
...
async def set_react_id(user, data):
    if not client.datafile:
        return (
            f"Please set a session data file when running the bot to add to watchlist."
        )
