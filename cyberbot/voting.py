...
class Voter:
    ...
    )  # positions that this user has voted for
...
class Voting:
    ...
    def handle_vote(self, user, params):
        ...
        for voter in self.voters:
            if voter.user == user.id:
                if (
                    voter.positions == self.positions_to_elect
                ):  # voter has voted for every needed position
    ...
    def get_nominations(self):
        ...
        all_noms = (
            []
        )  # list of {"position": "president", "users": [{"user": dayt0n#5130", "noms": 2}]}
    ...
    async def handle_nominate(self, user, params):
        ...
        for nom in self.nominations:
            ...
            if nom.user == nominee.id:
                ...
                await send_dm(
                    user,
                    f"{nominee.name} has already accepted the nomination for **{nom.position}**. To second this nomination, please reply with:\n`!nominate second`\nOtherwise, reply with:\n`!nominate cancel`",
                )
                while True:
        ...
        if not nomexists:
            ...
            if nominee != user:
                await send_dm(
                    nominee,
                    f"You have been nominated for the officer position: {position.capitalize()} of the UAH Cybersecurity Club!\nIf you accept, please indicate as such by replying with:\n`!nominate accept`\nIf you do **NOT** accept, reply with:\n`!nominate reject`",
                )
                while True:
            ...
            await send_dm(
                nominee,
                f"Please send a message stating your intentions as {position.capitalize()}. This is your chance to convince people to vote for you. You can only answer this prompt once.",
            )
        ...
        tosend = (
            tosend
            or f"{nominee.name} has **accepted** your nomination for {position.capitalize()}!"
        )
        self.users_waiting_for_nom.remove(user.id)
        self.__update_election_session_data()
        return tosend
    ...
    async def start_nomination(self, pieces):
        ...
        if not self.__does_election_exist_in_session():
            client.update_session(
                ...
                {
                    self.name: [
                        self.positions_to_elect,
                        self.nominations,
                        self.candidates,
                        self.voters,
                        self.users_waiting_for_nom,
                        self.usersWaitingForStatement,
                        self.eligible_members,
                        self.election_started,
                        self.nomination_started,
                    ]
                },
                append=True,
            )
    ...
    def stop_nomination(self, pieces=None):
        tosend = (
            ...
            if self.nomination_started
            else "The nomination period has not started"
        )
        if pieces == "clear":
            self.positions_to_elect.clear()
    ...
    def __get_nomination_start_string(self):
        ...
        for member in self.eligible_members:
            ...
            memberlist += (
                f"\t- <@{member}> (`{thisMember.name}#{thisMember.discriminator}`)\n"
            )
        position_list = ""
        for pos in self.positions_to_elect:
